...
class TestCustomerStorage(unittest.TestCase):
    def test_get_customer_by_id(self):
        ...
        self.assertEqual(result.id, "01F8MECHZX3TBDSZ7XD96VR2H5")
        self.assertEqual(result.name, "John Doe")
        self.assertEqual(result.email, "johndoe@example.com")
        self.assertEqual(result.created_at.strftime('%Y-%m-%d %H:%M:%S'), '2024-01-01 12:00:00')
        self.assertIsNone(result.updated_at)
        
    def test_get_all_customer_success(self):
        mock_db = MagicMock()
        mock_cursor = MagicMock()
        mock_db.cursor.return_value.__enter__.return_value = mock_cursor
        ...
        self.assertEqual(result[0].id, "01F8MECHZX3TBDSZ7XD96VR2H5")
        self.assertEqual(result[1].id, "01F8MECHZX3TBDSZ7XD96VR2H6")
        
        self.assertEqual(result[0].name, "John Doe")
        self.assertEqual(result[1].name, "Jane Smith")
        
        self.assertEqual(result[0].email, "johndoe@example.com")
        self.assertEqual(result[1].email, "janesmith@example.com")

        self.assertEqual(result[0].created_at.strftime("%Y-%m-%d %H:%M:%S"), "2024-01-01 12:00:00")
        self.assertEqual(result[1].created_at.strftime("%Y-%m-%d %H:%M:%S"), "2024-01-02 12:00:00")
        
        self.assertIsNone(result[0].updated_at)
        self.assertEqual(result[1].updated_at.strftime("%Y-%m-%d %H:%M:%S"), "2024-01-01 12:00:00")
